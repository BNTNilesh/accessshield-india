...
| Aspect           | Web scanner                         | Document scanner                     | Mobile scanner                      |
| ---------------- | ----------------------------------- | ------------------------------------ | ----------------------------------- |
| **Trigger**      | `POST /api/v1/scans`                | `POST /api/v1/document-scans/upload` | `POST /api/v1/scans` (mobile asset) |
| **Queue**        | RabbitMQ `scans`                    | Redis `document-scan-jobs`           | RabbitMQ `mobile-scan-jobs`         |
| **Worker**       | api `dev:worker`                    | ai-service lifespan consumer         | mobile-scanner                      |
| **Engine**       | Playwright, axe-core, IS17802, GIGW | PDF/DOCX/PPTX/XLSX engines + Tika    | TraversalAgent + MobileRuleEngine   |
| **Progress key** | `scan:progress:{id}`                | DB `progress_percent`                | `mobile-scan:progress:{id}`         |
| **Artifacts**    | Screenshots → S3                    | Original file → S3                   | APK/IPA → S3, screen shots          |
...
Requires RabbitMQ + Redis + Postgres.
...
The web worker **publishes** optional messages to RabbitMQ queues `ai-alt-text` and `ai-fix` after finding image/alt-text violations. **No consumer exists in-repo** for these queues.

On-demand AI from the issues UI uses **synchronous HTTP**:

`issues route` → [`apps/api/src/lib/ai-client.ts`](../../apps/api/src/lib/ai-client.ts) → ai-service

**Architectural note:** Consider consolidating on either queue-based or HTTP-based AI enrichment to avoid duplicate patterns.
...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| Web scans      | Multiple `dev:worker` / Lambda instances consuming `scans` queue                                                                                  |

...
| **api scan worker** | Node + Playwright + axe-core | —                   | Consumes RabbitMQ `scans` queue; crawls and audits websites |
...
| **RabbitMQ**        | 3.13                         | 5672                | Web and mobile scan job queues                              |
...
| Supabase Realtime        | Dashboard live scan status updates                         |
| S3 presigned URLs        | Document download for AI worker; screenshot/report storage |

---

## Known Architecture Notes (for reviewers)

1. **Dual migration paths:** Drizzle migrations in `packages/db/migrations/` and Supabase SQL in `supabase/migrations/` — align before production cutover.
2. **AI enrichment queues:** Web worker publishes to `ai-alt-text` / `ai-fix` RabbitMQ queues; on-demand AI from the issues UI uses HTTP to ai-service instead.

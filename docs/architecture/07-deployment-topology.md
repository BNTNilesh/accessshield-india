...
**Local development** runs application code on the host via `pnpm dev`, with backing services in **Docker Compose**. **Production target** deploys web to **Vercel**, API and workers to **AWS ap-south-1 (Mumbai)**, auth/DB to **Supabase**, and static assets to **S3 + CloudFront**.

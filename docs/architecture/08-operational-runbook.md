...
# Edit: Supabase URL/keys, DATABASE_URL, REDIS_URL, RABBITMQ_URL
...
# 6. Supabase auth hook (one-time, in Supabase SQL Editor)
# Run: packages/db/seed/supabase-auth-hook.sql
# Enable: Dashboard → Authentication → Hooks → Custom Access Token

# 7. Create sysadmin user (optional)
bash scripts/seed-sysadmin.sh

# 8. Playwright browsers (if not already from postinstall)
